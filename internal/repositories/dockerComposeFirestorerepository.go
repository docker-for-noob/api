...
func (repo *dockerComposeFirestore) ReadAll(firstItemRank int, userId string) ([]domain.DockerCompose, error) {
	ctx := context.Background()
	client := CreateClient(ctx)
	defer client.Close()

	dockerComposeSearch := client.Collection("Users").Doc(userId).Collection("Files").OrderBy("createdAt", firestore.Desc).Limit(25).Offset(firstItemRank).Documents(ctx)
...
func (repo *dockerComposeFirestore) Read(id string, userId string) (domain.DockerCompose, error) {
	ctx := context.Background()
	client := CreateClient(ctx)
	defer client.Close()

	dockerComposeSearch, err := client.Collection("Users").Doc(userId).Collection("Files").Doc(id).Get(ctx)
	if err != nil {
		return domain.DockerCompose{}, errors.New(apperrors.Internal, err, "An internal error occurred", "")
	}
...
func (repo *dockerComposeFirestore) Create(dockerCompose domain.DockerCompose, userId string,id string) (domain.DockerCompose, error) {
...
func (repo *dockerComposeFirestore) Update(dockerCompose domain.DockerCompose, userId string) (domain.DockerCompose, error) {
	ctx := context.Background()
	client := CreateClient(ctx)
	defer client.Close()
...
func (repo *dockerComposeFirestore) Delete(id string, userId string) (bool, error) {
	ctx := context.Background()
	client := CreateClient(ctx)
	defer client.Close()

	_, err := client.Collection("Users").Doc(userId).Collection("Files").Doc(id).Delete(ctx)
	if err != nil {
